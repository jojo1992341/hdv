...
function _buildResRow(res) {
    ...
    // Prix moyen importé (stocké sous avg_{id} en localStorage)
    const avgRaw  = localStorage.getItem('avg_' + res.id_res);
    const avgVal  = avgRaw !== null ? parseInt(avgRaw, 10) : null;
    const avgCell = avgVal !== null && avgVal > 0
        ? `<span class="res-avg-price" title="Prix moyen importé">${avgVal.toLocaleString('fr-FR')} K</span>`
